...
module.exports = {
  ...
  //TODO-- add preview snippets of post bodies
   index(source){
  ...
  //TODO-- extra configurations passed in as a config object
  roll(source, config){
    ...
    index.forEach(post => {
      ...
      const finalPostPath = source + '/' + post.year + '/' + post.month + '/' + post.day + '/' + post.title + '/index.html';
      ...
      if (config.snippet) {
        function snip(data){
          ...
          snippet = html.substring(p, config.snippetChars + p) + '</p>';
        };
