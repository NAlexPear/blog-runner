...
module.exports = {
  ...
  mixin(source){
    ...
    const jsFilesArray = glob.sync(paths.mixins + '/*.js');

    jsFilesArray.forEach(file => {
      const name = filenameParse(file);
      const mixin = require(file);
      //save the result of the output() method in each mixin
      const data = mixin.output();
      //create new HTML file for _includes from mixin file output
      fs.writeFileSync(paths.includes + '/' + name + '.html', data);
    });
  },
  ...
  build(source){
    ...
    //move any folder without an undescore directly to _site
    const dirs = fs.readdirSync(source);
    dirs.forEach(dir => {
      if(dir.includes('_') === false){
        const path = source + '/' + dir;
        ncp(path, sitePath + '/' + dir, (err) => {
          if (err) console.log(err);
        });
        console.log('moved directory: ' + dir);
      }
    });
  }
}
